...
def mean_shift_from_kde(X, bandwidth=0.1, tol=1e-3, max_iter=300):
    ...
    # ---------------------------
    # Step 1: Mean shift for each point
    # ---------------------------
    for i in range(N):
        x = X[i].copy()
    ...
    # ---------------------------
    # Step 3: Assign each point to nearest mode
    # ---------------------------
    assignments = []
    for x in X:
...
def plot_results(embedding, meci_emb, modes, density_values, meci_files, out_dir):
    ...
    # MECIs
    for i in range(meci_emb.shape[0]):
        label = os.path.basename(meci_files[i])
