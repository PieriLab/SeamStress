...
# Map SMILES to human-readable names
SMILES_TO_NAME = {
...
def load_centroids(centroids_dir: Path) -> dict:
    ...
    for xyz_file in centroids_dir.glob("*.xyz"):
        ...
        centroids[smiles] = {'coords': np.array(coords), 'name': name}
    return centroids
...
def main():
    ...
    FEATURE_TYPES = [("cartesian_aligned", coords_to_cartesian), ("inverse_distance", coords_to_inverse_distance)]
    ...
    families = [f"family_{i}" for i in range(1, 10)]
    all_data = {}
    ...
    for threshold, thresh_name in THRESHOLDS:
        ...
        for feature_name, feature_func in FEATURE_TYPES:
            ...
            result = compute_all_embeddings(aligned_dir, centroids, threshold, families, feature_name, feature_func)
            if result:
                all_data[key] = result
    ...
    print("\n" + "=" * 70)
    print("Done! Open analysis_output/explorer.html in your browser")
    print("=" * 70)
